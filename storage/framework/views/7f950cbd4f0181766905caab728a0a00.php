...
<body class="font-sans antialiased">
    <div class="w-full max-w-7xl mx-auto py-8 px-4">
        ...
        <div class="approval-card rounded-2xl p-8">
            ...
            <div id="alerte-solde" class="hidden bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
                <div class="flex items-center">
                    <i class="fas fa-exclamation-triangle text-yellow-600 text-xl mr-3"></i>
                    <div>
                        <h4 class="font-semibold text-yellow-800">Solde insuffisant pour les frais</h4>
                        <p class="text-yellow-700 text-sm mt-1">
                            Le solde du groupe (<span id="solde-actuel-groupe">0.00 USD</span>) est insuffisant pour couvrir les frais de dossier (<span id="frais-totaux-calcul">0.00 USD</span>). 
                            Le crédit pourra être accordé, mais les frais ne seront pas prélevés.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form action="<?php echo e(route('credits.process-approval-groupe', $credit->id)); ?>" method="POST" class="mb-8" id="approval-form">
                ...
                <div class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200 mb-6">
                    ...
                    <div id="calculs-groupe" class="mt-6 space-y-4 hidden">
                        ...
                        <!-- Section Caution Totale -->
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-yellow-50 rounded-lg p-4 text-center border border-yellow-200">
                                <p class="text-sm text-gray-600">Caution Totale Groupe</p>
                                <p class="text-xl font-bold text-yellow-600" id="total-caution-groupe">0.00 USD</p>
                                <p class="text-xs text-yellow-600 mt-1">20% du total accordé aux membres</p>
                            </div>
                            <div class="bg-blue-50 rounded-lg p-4 text-center border border-blue-200">
                                <p class="text-sm text-gray-600">Total Frais + Caution</p>
                                <p class="text-xl font-bold text-blue-600" id="total-frais-caution">0.00 USD</p>
                                <p class="text-xs text-blue-600 mt-1">Montant total à bloquer</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            >
                ...
                <?php $__currentLoopData = $agents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $agent): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($agent->id); ?>" 
                        <?php if(old('agent_id', $credit->agent_id) == $agent->id): ?> selected <?php endif; ?>>
            ...
            >
                ...
                <?php $__currentLoopData = $superviseurs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $superviseur): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($superviseur->id); ?>"
                        <?php if(old('superviseur_id', $credit->superviseur_id) == $superviseur->id): ?> selected <?php endif; ?>>
...
<script>
    ...
    // Tableau des frais selon les tranches (sans frais d'adhésion)
    const fraisParTranche = {
    ...
    // Fonction pour calculer les frais d'un membre
    function calculerFraisMembre(montant) {
        const montantArrondi = Math.floor(montant / 50) * 50;
        let tranche = montantArrondi;
        
        if (tranche < 50) tranche = 50;
        if (tranche > 500) tranche = 500;
        
        return fraisParTranche[tranche] || fraisParTranche[500];
    }
    ...
    function calculerFraisEtAlerte() {
        ...
        document.querySelectorAll('.montant-membre').forEach(input => {
            ...
            if (montantMembre > 0) {
                totalSaisiMembres += montantMembre;
                
                // Calculer les frais du membre
                const fraisMembre = calculerFraisMembre(montantMembre);
                const cautionMembre = montantMembre * 0.20;
                
                totalFrais += fraisMembre.total_frais;
                totalCaution += cautionMembre;
                
                // Mettre à jour l'affichage pour ce membre
                document.getElementById(`frais-dossier-${membreId}`).textContent = fraisMembre.dossier.toFixed(2) + ' USD';
                document.getElementById(`frais-alerte-${membreId}`).textContent = fraisMembre.alerte.toFixed(2) + ' USD';
                document.getElementById(`frais-carnet-${membreId}`).textContent = fraisMembre.carnet.toFixed(2) + ' USD';
                document.getElementById(`caution-${membreId}`).textContent = cautionMembre.toFixed(2) + ' USD';
                document.getElementById(`total-frais-${membreId}`).textContent = fraisMembre.total_frais.toFixed(2) + ' USD';
            } else {
                // Réinitialiser l'affichage si montant = 0
                document.getElementById(`frais-dossier-${membreId}`).textContent = '0.00 USD';
                document.getElementById(`frais-alerte-${membreId}`).textContent = '0.00 USD';
                document.getElementById(`frais-carnet-${membreId}`).textContent = '0.00 USD';
                document.getElementById(`caution-${membreId}`).textContent = '0.00 USD';
                document.getElementById(`total-frais-${membreId}`).textContent = '0.00 USD';
            }
        });

        if (montantTotalGroupe > 0 || totalSaisiMembres > 0) {
            // Montant avec intérêt
            const montantAvecInteret = montantTotalGroupe * coefficientGroupe;
            const remboursementHebdo = montantAvecInteret / dureeSemaines;
            
            // Afficher les calculs
            document.getElementById('montant-total-interet').textContent = montantAvecInteret.toFixed(2) + ' USD';
            document.getElementById('remboursement-hebdo-total').textContent = remboursementHebdo.toFixed(2) + ' USD';
            document.getElementById('total-frais-groupe').textContent = totalFrais.toFixed(2) + ' USD';
            document.getElementById('total-caution-groupe').textContent = totalCaution.toFixed(2) + ' USD';
            document.getElementById('total-frais-caution').textContent = (totalFrais + totalCaution).toFixed(2) + ' USD';
            
            // Gérer l'alerte solde insuffisant
            const alerteSolde = document.getElementById('alerte-solde');
            if (soldeGroupe < totalFrais) {
                alerteSolde.classList.remove('hidden');
                document.getElementById('solde-actuel-groupe').textContent = soldeGroupe.toFixed(2) + ' USD';
                document.getElementById('frais-totaux-calcul').textContent = totalFrais.toFixed(2) + ' USD';
            } else {
                alerteSolde.classList.add('hidden');
            }
            
            document.getElementById('calculs-groupe').classList.remove('hidden');
        } else {
            document.getElementById('calculs-groupe').classList.add('hidden');
            document.getElementById('alerte-solde').classList.add('hidden');
        }
    }
    ...
    function validateApprovalGroupe() {
        ...
        document.querySelectorAll('.montant-membre').forEach(input => {
            ...
            if (montant > 0) {
                membresAvecMontant++;
                const fraisMembre = calculerFraisMembre(montant);
                totalFrais += fraisMembre.total_frais;
                totalCaution += montant * 0.20;
            }
        });
        
        const difference = totalSaisiMembres - montantTotalGroupe;
        const soldeInsuffisant = soldeGroupe < totalFrais;
        ...
        let messageConfirmation = `Êtes-vous sûr de vouloir approuver ce crédit groupe ?\n\n` +
            `📊 Montant total: ${montantTotalGroupe.toFixed(2)} USD\n` +
            `👥 Réparti entre: ${membresAvecMontant} membre(s)\n` +
            `💰 Frais totaux: ${totalFrais.toFixed(2)} USD\n` +
            `🔒 Caution totale: ${totalCaution.toFixed(2)} USD\n` +
            `✅ Équilibre parfait`;
        
        if (soldeInsuffisant) {
            messageConfirmation += `\n\n⚠️ ATTENTION: Solde insuffisant pour les frais\n` +
                `Solde groupe: ${soldeGroupe.toFixed(2)} USD | Frais requis: ${totalFrais.toFixed(2)} USD\n` +
                `Le crédit sera accordé mais les frais ne seront pas prélevés.`;
        }
...
</html><?php /**PATH C:\laragon\www\tumainiletusystem2.0\resources\views/credits/approval-groupe-final.blade.php ENDPATH**/ ?>