...
<body class="font-sans antialiased">
        ...
        <div class="approval-card rounded-2xl p-8">
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200 mb-8">
                <h2 class="text-2xl font-semibold text-gray-800 mb-4">
                    Groupe: {{ $credit->groupeCompte->nom_groupe ?? 'Nom non disponible' }}
                </h2>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        <p class="text-sm text-gray-600">Nombre de membres</p>
                        <p class="text-xl font-bold text-gray-800">{{ $credit->groupeCompte->membres->count() ?? 0 }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Montant demandé</p>
                        <p class="text-xl font-bold text-blue-600">
                            {{ number_format(floatval($variable ?? 0), 2, ',', ' ') }} {{ $credit->groupeCompte->devise ?? 'USD' }}
                        </p>
                    </div>
                    <div>
            ...
            <form action="{{ route('credits.process-approval-groupe', $credit->id) }}" method="POST" class="mb-8" id="approval-form">
                ...
                <div class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200 mb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <span class="text-gray-500 font-medium">{{ $credit->groupeCompte->devise ?? 'USD' }}</span>
                            </div>
                            <input 
                                type="number" 
                                ...
                                step="0.01"
                                min="0.01"
                                max="{{ $credit->montant_demande * 1.5 }}"
                    ...
                    <div class="space-y-4">
                        ...
                        @foreach($credit->groupeCompte->membres as $membre)
                        <div class="membre-card bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 border-l-blue-400">
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-gray-800">{{ $membre->nom }} {{ $membre->prenom }}</p>
                                        <p class="text-xs text-gray-600">{{ $membre->compte->numero_compte ?? 'N/A' }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm text-gray-600">Solde: {{ number_format($membre->compte->solde ?? 0, 2, ',', ' ') }} {{ $credit->groupeCompte->devise ?? 'USD' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="relative flex-1">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <span class="text-gray-500 text-sm">{{ $credit->groupeCompte->devise ?? 'USD' }}</span>
                                    </div>
                                    <input 
                                        type="number" 
                    ...
                    <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <div class="flex items-center justify-between">
                            <span class="text-sm text-blue-800 font-medium">
                                ...
                                {{ $credit->groupeCompte->devise ?? 'USD' }}
                            </span>
    ...
    <script>
        const membres = @json($credit->groupeCompte->membres ?? []);
        const devise = "{{ $credit->groupeCompte->devise ?? 'USD' }}";
