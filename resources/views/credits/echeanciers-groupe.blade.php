...
<head>
    ...
    <title>Échéanciers du Crédit Groupe - {{ $credit->compte->nom }}</title>
    <style>
...
<body>
    ...
    <div class="screen-actions no-print">
        ...
        <a href="{{ route('credits.details-groupe', $credit->id) }}" class="btn btn-success">
            <i class="fas fa-chart-pie"></i> Retour aux Détails
        </a>
        <a href="{{ route('comptes.details', $credit->compte_id) }}" class="btn btn-secondary">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-top: 15px;">
            <div style="text-align: center; padding: 15px; background: #e3f2fd; border-radius: 8px;">
                ...
                <div style="font-size: 18px; font-weight: bold; color: #1976d2;">
                    {{ number_format($credit->montant_total, 2, ',', ' ') }} USD
                </div>
            </div>
            <div style="text-align: center; padding: 15px; background: #e8f5e8; border-radius: 8px;">
                ...
                <div style="font-size: 18px; font-weight: bold; color: #388e3c;">
                    {{ number_format($credit->remboursement_hebdo_total, 2, ',', ' ') }} USD
                </div>
            </div>
            <div style="text-align: center; padding: 15px; background: #f3e5f5; border-radius: 8px;">
                ...
                <div style="font-size: 18px; font-weight: bold; color: #7b1fa2;">
                    {{ $credit->date_octroi->copy()->addWeeks(2)->format('d/m/Y') }}
                </div>
            </div>
    ...
    @foreach($echeanciers as $compteId => $echeancierMembre)
        @php
        ...
        <div class="page-break">
            ...
            <div class="ref-date">
                <div>RÉFÉRENCE : ÉCH-G{{ $credit->id }}-M{{ $compteId }}-{{ date('Ymd') }}</div>
            ...
            <div class="client-info">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item">
                        ...
                        <span>{{ number_format($montantAccordeMembre, 2, ',', ' ') }} USD</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">Montant Total :</span>
                        <span>{{ number_format($montantTotalMembre, 2, ',', ' ') }} USD</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">Remb. Hebdo :</span>
                        <span>{{ number_format($montantHebdo, 2, ',', ' ') }} USD</span>
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        ...
                        <span>{{ $credit->date_echeance->format('d/m/Y') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="total-section">
                <div class="total-grid">
                    <div class="total-item">
                        ...
                        <div class="total-value">{{ number_format($montantAccordeMembre, 2, ',', ' ') }} USD</div>
                    </div>
                    <div class="total-item">
                        <div>Total Intérêts</div>
                        <div class="total-value">{{ number_format($montantTotalMembre - $montantAccordeMembre, 2, ',', ' ') }} USD</div>
                    </div>
                    <div class="total-item">
                        <div>Montant Total</div>
                        <div class="total-value">{{ number_format($montantTotalMembre, 2, ',', ' ') }} USD</div>
                    </div>
                    <div class="total-item">
            ...
            <div style="margin-top: 12px; padding: 8px; background-color: #fff3cd; border: 1px solid #ffeaa7; border-radius: 4px; font-size: 9px;">
                ...
                • La caution de {{ number_format($montantAccordeMembre * 0.20, 2, ',', ' ') }} USD sera débloquée après remboursement complet<br>
            ...
            <div class="signature-section">
                <div class="signature">
                    ...
                    <div style="margin-top: 35px;">{{ $membreNom }}</div>
                </div>
                <div class="signature">
            ...
            <div class="footer">
                <div class="separator"></div>
                Document généré le {{ now()->format('d/m/Y à H:i') }} | Page {{ $loop->iteration }}/{{ $loop->count }} | Tumaini Letu asbl
            </div>
        </div>
