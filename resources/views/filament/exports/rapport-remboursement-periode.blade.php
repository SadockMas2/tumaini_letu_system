...
<body>
    ...
    <div class="header">
        <div class="logo">
            <img src="{{ $logo_base64 }}" alt="TUMAINI LETU asbl">
        </div>
    ...
    <div class="ref-date">
        <div>N/REF : REMB-{{ strtoupper($periode) }}-{{ \Carbon\Carbon::now()->format('Ymd-His') }}</div>
        <div>Période : {{ $date_debut }} au {{ $date_fin }}</div>
        <div>Généré le : {{ $date_rapport }}</div>
    </div>
    ...
    <div class="main-title">
        <h1>RAPPORT DE REMBOURSEMENT PAR PÉRIODE {{ strtoupper($titre_periode) }}</h1>
        <div class="subtitle">
            {{ $type_credit }} | {{ count($remboursements) }} remboursements planifiés
        </div>
    </div>
    ...
    <div class="totals-grid">
        <div class="total-card">
            ...
            <div class="total-value montant">{{ number_format($totaux['total_remboursement'], 2) }} $</div>
        </div>
        <div class="total-card">
            <div class="total-label">Capital Total</div>
            <div class="total-value montant">{{ number_format($totaux['total_capital'], 2) }} $</div>
        </div>
        <div class="total-card">
            <div class="total-label">Intérêts Totaux</div>
            <div class="total-value montant">{{ number_format($totaux['total_interets'], 2) }} $</div>
        </div>
        <div class="total-card">
            <div class="total-label">Nombre de Périodes</div>
            <div class="total-value">{{ $totaux['nombre_periodes'] }}</div>
        </div>
        <div class="total-card">
            <div class="total-label">Crédits Concernés</div>
            <div class="total-value">{{ $totaux['nombre_credits'] }}</div>
        </div>
        <div class="total-card">
            <div class="total-label">% Capital Moyen</div>
            <div class="total-value">{{ number_format($totaux['moyenne_capital'], 1) }}%</div>
        </div>
        <div class="total-card">
            <div class="total-label">% Intérêts Moyen</div>
            <div class="total-value">{{ number_format($totaux['moyenne_interets'], 1) }}%</div>
        </div>
        <div class="total-card">
            <div class="total-label">Période Type</div>
            <div class="total-value">{{ ucfirst($titre_periode) }}</div>
        </div>
    </div>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $item['periode'] }}</td>
                <td class="nowrap">{{ $item['date_periode']->format('d/m/Y') }}</td>
                <td>{{ $item['numero_compte'] }}</td>
                <td>
                    @if($item['type_credit'] === 'individuel')
                        <span class="badge badge-success">IND</span>
                    @else
                        <span class="badge badge-warning">GRP</span>
                    @endif
                </td>
                <td style="font-size: 9px;">{{ Str::limit($item['nom_complet'], 25) }}</td>
                <td class="text-right montant">{{ number_format($item['montant_total'], 2) }} $</td>
                <td class="text-right montant">{{ number_format($item['capital'], 2) }} $</td>
                <td class="text-right montant">{{ number_format($item['interets'], 2) }} $</td>
                <td>
                    @if(isset($item['agent_nom']) && $item['agent_nom'])
                        <span class="agent-name">{{ Str::limit($item['agent_nom'], 20) }}</span>
                    @elseif(isset($item['agent_id']))
                        <span class="badge badge-info">Agent #{{ $item['agent_id'] }}</span>
                    @else
                ...
                <td class="text-center">
                    ...
                    @elseif($item['statut'] === 'En retard')
                        <span class="badge badge-danger">Retard</span>
                    @elseif($item['statut'] === 'En cours')
    ...
    <div class="summary">
        <div class="summary-item">
            ...
            <span class="summary-value montant">{{ number_format($totaux['total_remboursement'], 2) }} USD</span>
        </div>
        <div class="summary-item">
            <span class="summary-label">Dont capital :</span>
            <span class="summary-value montant">{{ number_format($totaux['total_capital'], 2) }} USD</span>
        </div>
        <div class="summary-item">
            <span class="summary-label">Dont intérêts :</span>
            <span class="summary-value montant">{{ number_format($totaux['total_interets'], 2) }} USD</span>
        </div>
        <div class="summary-item">
            <span class="summary-label">Nombre de crédits :</span>
            <span class="summary-value">{{ $totaux['nombre_credits'] }}</span>
        </div>
        <div class="summary-item">
            <span class="summary-label">Nombre de périodes :</span>
            <span class="summary-value">{{ $totaux['nombre_periodes'] }}</span>
        </div>
    </div>
