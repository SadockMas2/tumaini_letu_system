...
<head>
    ...
    <title>Évolution des Coffres - {{ $evolution['periode']['debut'] }} à {{ $evolution['periode']['fin'] }}</title>
    <style>
...
<body>
    ...
    <div class="header">
        ...
        <div style="text-align: right;">
            <div>Période : {{ $evolution['periode']['debut'] }} à {{ $evolution['periode']['fin'] }}</div>
    ...
    <div style="margin-bottom: 15px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 15px;">
            <div style="border: 1px solid #000; padding: 8px; text-align: center;">
                ...
                <div class="montant">{{ number_format($evolution['total_usd_debut'], 2) }} $</div>
            </div>
            <div style="border: 1px solid #000; padding: 8px; text-align: center;">
                <div style="font-size: 9px;">USD Fin</div>
                <div class="montant">{{ number_format($evolution['total_usd_fin'], 2) }} $</div>
            </div>
            <div style="border: 1px solid #000; padding: 8px; text-align: center;">
                <div style="font-size: 9px;">Évolution USD</div>
                <div class="montant {{ $evolution['evolution_usd'] >= 0 ? 'evolution-positive' : 'evolution-negative' }}">
                    {{ number_format($evolution['evolution_usd'], 2) }} $
                </div>
            </div>
            <div style="border: 1px solid #000; padding: 8px; text-align: center;">
                <div style="font-size: 9px;">Taux USD</div>
                <div class="{{ $evolution['total_usd_debut'] != 0 ? ($evolution['evolution_usd'] >= 0 ? 'evolution-positive' : 'evolution-negative') : '' }}">
                    {{ $evolution['total_usd_debut'] != 0 ? number_format(($evolution['evolution_usd'] / $evolution['total_usd_debut']) * 100, 2) : 'N/A' }}%
                </div>
            </div>
            
            <div style="border: 1px solid #000; padding: 8px; text-align: center;">
                <div style="font-size: 9px;">CDF Début</div>
                <div class="montant">{{ number_format($evolution['total_cdf_debut'], 0) }} FC</div>
            </div>
            <div style="border: 1px solid #000; padding: 8px; text-align: center;">
                <div style="font-size: 9px;">CDF Fin</div>
                <div class="montant">{{ number_format($evolution['total_cdf_fin'], 0) }} FC</div>
            </div>
            <div style="border: 1px solid #000; padding: 8px; text-align: center;">
                <div style="font-size: 9px;">Évolution CDF</div>
                <div class="montant {{ $evolution['evolution_cdf'] >= 0 ? 'evolution-positive' : 'evolution-negative' }}">
                    {{ number_format($evolution['evolution_cdf'], 0) }} FC
                </div>
            </div>
            <div style="border: 1px solid #000; padding: 8px; text-align: center;">
                <div style="font-size: 9px;">Taux CDF</div>
                <div class="{{ $evolution['total_cdf_debut'] != 0 ? ($evolution['evolution_cdf'] >= 0 ? 'evolution-positive' : 'evolution-negative') : '' }}">
                    {{ $evolution['total_cdf_debut'] != 0 ? number_format(($evolution['evolution_cdf'] / $evolution['total_cdf_debut']) * 100, 2) : 'N/A' }}%
                </div>
            </div>
        </div>
    </div>
    ...
    <table class="table">
        ...
        <tbody>
            @foreach($evolution['coffres'] as $coffre)
            <tr>
                <td>{{ $coffre['nom'] }}</td>
                <td>{{ $coffre['devise'] }}</td>
                <td class="text-right montant">
                    {{ number_format($coffre['solde_debut'], $coffre['devise'] === 'USD' ? 2 : 0) }}
                    {{ $coffre['devise'] === 'USD' ? '$' : 'FC' }}
                </td>
                <td class="text-right montant">
                    {{ number_format($coffre['solde_fin'], $coffre['devise'] === 'USD' ? 2 : 0) }}
                    {{ $coffre['devise'] === 'USD' ? '$' : 'FC' }}
                </td>
                <td class="text-right montant {{ $coffre['evolution'] >= 0 ? 'evolution-positive' : 'evolution-negative' }}">
                    {{ number_format($coffre['evolution'], $coffre['devise'] === 'USD' ? 2 : 0) }}
                    {{ $coffre['devise'] === 'USD' ? '$' : 'FC' }}
                </td>
                <td class="text-center {{ $coffre['solde_debut'] != 0 ? ($coffre['evolution'] >= 0 ? 'evolution-positive' : 'evolution-negative') : '' }}">
                    @if($coffre['solde_debut'] != 0)
                        {{ number_format(($coffre['evolution'] / $coffre['solde_debut']) * 100, 2) }}%
                    @else
                        N/A
                    @endif
                </td>
                <td class="text-right montant">
                    {{ number_format($coffre['entrees'], $coffre['devise'] === 'USD' ? 2 : 0) }}
                </td>
                <td class="text-right montant">
                    {{ number_format($coffre['sorties'], $coffre['devise'] === 'USD' ? 2 : 0) }}
                </td>
        ...
        <div style="display: flex; justify-content: space-around; margin-top: 15px;">
            ...
            <div>
                ...
                <div style="width: 200px; height: 150px; border: 1px solid #000; position: relative;">
                    @if($maxCDF > 0)
                        <div style="position: absolute; bottom: 0; left: 25px; width: 40px; 
                             height: {{ ($evolution['total_cdf_debut'] / $maxCDF) * 100 }}%; 
                             background-color: #e74c3c;"></div>
                        <div style="position: absolute; bottom: 0; right: 25px; width: 40px; 
                             height: {{ ($evolution['total_cdf_fin'] / $maxCDF) * 100 }}%; 
    ...
    <div style="margin-top: 30px; padding-top: 10px; border-top: 1px solid #000;">
        ...
        <div style="margin-top: 10px; font-size: 11px;">
            @if($evolution['evolution_usd'] > 0)
                <p>✓ Progression positive des coffres USD : +{{ number_format($evolution['evolution_usd'], 2) }} $</p>
            @elseif($evolution['evolution_usd'] < 0)
                <p>⚠ Régression des coffres USD : {{ number_format($evolution['evolution_usd'], 2) }} $</p>
            @else
                <p>↔ Stabilité des coffres USD</p>
            @endif
            
            @if($evolution['evolution_cdf'] > 0)
                <p>✓ Progression positive des coffres CDF : +{{ number_format($evolution['evolution_cdf'], 0) }} FC</p>
            @elseif($evolution['evolution_cdf'] < 0)
                <p>⚠ Régression des coffres CDF : {{ number_format($evolution['evolution_cdf'], 0) }} FC</p>
            @else
